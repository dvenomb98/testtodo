...
const Login = () => {
  ...
  const redirectAfterLogin = () => {
    setLoginSuccess(
      "Login completed. Please wait, redirecting..."
    );
    setTimeout(() => {
      router.push("/main")
    }, 1500)
  ...
  return (
    <Modal openModal={openLoginModal} modalType={(loginSuccess || registerSuccess) ? ModalType.Success : ModalType.Default} onModalEnter={() => {setLoginError(null);setRegisterError(null);setLoginSuccess(null);setRegisterSuccess(null)}}>
                ...
                <Form className="flex flex-col gap-5">
                  ...
                  <p className="text-white">{loginSuccess}</p>
                    ...
                    <>
                      ...
                      {loginError && (
                        <p className={errorPopper}>{loginError}</p>
                      )}

                      <button
                        type="submit"
                        className={`${pinkGradientButton} text-white w-full`}
                      >
                        {isSubmitting ? (
          ...
          <>
            ...
            <Formik
              ...
              onSubmit={async (values: RegisterProps) => {
                ...
                isSuccess &&
                  setRegisterSuccess("Registration completed. Please sign in.");
              }}
              key={"registerFormik"}
            >
              {({ errors, touched, isSubmitting }) => (
                <Form className="flex flex-col">
                  {registerSuccess ? (
                    ...
                    <>
                      ...
                      {registerError && (
                        <p className={errorPopper}>{registerError}</p>
                      )}
                      <button
                        type="submit"
