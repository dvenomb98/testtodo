...
import { errorPopper, pinkGradientButtonSecondary } from '../Styles/CustomStyles';
...
const DeleteTask: React.FC<DeleteTaskProps> = ({ task }) => {
  ...
  const deleteTask = async (id: string) => {
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        const newTasks = [...sfDoc.data().tasks.filter((t: Task) => t.id !== id)];
  ...
  return (
    <Modal
      ...
      modalType={!deleted ? ModalType.Dark : ModalType.Success}
      onModalEnter={() => setFormError('')}
    >
      <div className="p-5 text-white">
        {deleted ? (
          ...
          <div className="flex flex-col gap-3">
            <p>Are you sure you want delete this task? Action cannot be returned!</p>
            ...
            >
              {loadingDelete ? <Loader loading={loadingDelete} /> : 'Confirm'}
            </button>
          </div>
        )}
      </div>
